...
class config:
    ...
    value_model_type: str = '7B'  # 3B, 7B, 13B, 70B
    reward_model_type: str = '7B'  # 3B, 7B, 13B, 70B
    max_seq_len: int = 512

    ref_ckpt_file: str = './checkpoints/sft/7B/7B-steps-2500.pth'  # load fine-tuned checkpoint for reference model
    reward_ckpt_file: str = './checkpoints/rm/7B/7B-steps-1285.pth'  # load RM checkpoint
    reward_normalizer_ckpt_file: str = './checkpoints/rm/normalizer/normalizer-samples-32928.pth'  # load RM normalizer checkpoint
    policy_ckpt_file: str = './checkpoints/sft/7B/7B-steps-2500.pth'  # default same as fine-tuned checkpoint
    value_ckpt_file: str = './checkpoints/rm/7B/7B-steps-1285.pth'  # default same as RM checkpoint
    tokenizer_file: str = '/home/michael/models/meta_llama2/tokenizer.model'  # load tokenizer model

    agent_ckpt_file: str = None  # resume training

    # datasets
    train_prompt_datasources: Tuple[str] = ('./datasets/hh_rlhf_prompt_only/train.pkl',)
    val_prompt_datasources: Tuple[str] = ('./datasets/hh_rlhf_prompt_only/validation.pkl',)
    ...
    max_prompt_len: int = 256  # limit the maximum length of prompts in the prompt dataset
    ...
    max_gen_len: int = 256  # cut maximum completion tokens in the batch, this does not necessarily apply to every episode
    clip_reward: float = 2.0  # if > 0, clip (normalized) environment reward in the range of [-clip_reward, clip_reward]
    selfplay_log_interval: int = 10  # frequency to log episode metrics (reward, steps etc.) to tensorboard
    selfplay_sample_interval: int = 100  # frequency to log selfplay generated text to tensorboard

    # PPO learning
    max_episodes: int = int(2e5)  # total number of training episodes
    train_rollout_size: int = 256  # how many training selfplay episodes to generate per iteration
    ppo_batch_size: int = 32
    ppo_update_epochs: int = 4  # PPO update epoch
    policy_micro_bs: int = 2  # micro batch size for policy
    value_micro_bs: int = 2  # micro batch size for value
    ...
    # validation
    var_interval: int = 20  # N iterations, which is around N x train_rollout_size episodes
    val_rollout_size: int = 128
    ckpt_interval: int = 20  # N iterations, which is around N x train_rollout_size episodes

    # frozen the first N decoder layers and make the last M-N decoder layers along with the output layer fully-trainable
    policy_frozen_layers: int = 16
    value_frozen_layers: int = 16
    ...
    policy_init_lr: float = 1.04e-6  # initial learning rate
    policy_max_lr: float = 1.04e-5  # max learning rate after warm up
    policy_min_lr: float = 1.04e-5  # min learning rate after decay
    policy_lr_warmup_steps: int = 300
