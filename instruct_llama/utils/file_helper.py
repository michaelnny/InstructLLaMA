...
def find_certain_files_under_dir(
    root_dir: str, file_type: str = ".txt"
) -> Iterable[str]:
...
def read_txt_file(input_file: str, is_chinese: bool = False) -> Text:
    ...
    if (
        ...
        or not input_file.endswith(".txt")
    ):
        return None
...
def read_json_file(input_file: str) -> Iterable[Mapping[Text, Any]]:
    ...
    if (
        not os.path.exists(input_file)
        or not os.path.isfile(input_file)
        or not input_file.endswith(".json")
    ):
        return None
...
def read_jsonl_file(input_file: str) -> Iterable[Mapping[Text, Any]]:
    ...
    if (
        not os.path.exists(input_file)
        or not os.path.isfile(input_file)
        or not input_file.endswith(".jsonl")
    ):
        return None
